...
# FULL PATCH for Recogito 1.9.7 Supabase null-return bug
RUN sed -i 's/\.data\[/\.data \&\& \.data\[/' recogito-server/create-default-groups.js && \
    sed -i 's/\.data.length/\.data \&\& \.data.length/' recogito-server/create-default-groups.js
